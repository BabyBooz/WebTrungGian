 ...
 * @author Fangl
 */
@WebServlet(name = "DetailAccount", urlPatterns = {"/detail-account"})
public class DetailAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DetailAccount</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DetailAccount at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String url = "admin/detail-account.jsp";
        try {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(account == null){
                url = String.format("/%s/manager-account", request.getContextPath());
            }else{
                request.setAttribute("ACCOUNT_DETAIL", account);
            }
        } catch (Exception e) {
